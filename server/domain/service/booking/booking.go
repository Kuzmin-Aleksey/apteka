...
type BookingRepo interface {
	...
	GetById(ctx context.Context, bookId int) (*models.Book, error)
	GetByStore(ctx context.Context, storeId int) ([]models.Book, error)
...
func (s *BookingService) ToBook(ctx context.Context, storeId int, dto *CreateBookDTO) (int, error) {
	book := &models.Book{
		StoreId:   storeId,
		CreatedAt: time.Now(),
		Status:    models.BookStatusCreated,
		Phone:     dto.Phone,
		Message:   dto.Message,
...
func (s *BookingService) GetByStore(ctx context.Context, storeId int) ([]models.Book, error) {
...
func (s *BookingService) StoreBookingDelay(newDelay time.Duration) error {
	if newDelay == 0 {
		return models.NewError(models.ErrInvalidRequest, "booking delay is zero")
	} else if newDelay == s.bookingDelay {
		return nil
	}

	if err := os.WriteFile(bookingDelayFilePath, []byte(newDelay.String()), os.ModePerm); err != nil {
		return models.NewError(models.ErrUnknown, err)
	}
